...
def run_meff_avg_charm(
    prefix, ii, nensemble, mp_string, T_strt, T_end,
    tag, baryon, jj, cov_enabled, two_state_enabled,kk, baryon_meson_h5_prefix
):
    ...
    if baryon == 'LAMBDA_C':
        ...
        c2pt, confs=h5_get_pp2pt(f'{baryon_meson_h5_prefix}/baryon_meson_data_h5/{tag}/c2pt_{modified_prefix}.wp.{baryon}.h5')
    else:
    ...
    c1s=mb_mr_data['c1'][f'ensemble{nensemble}'][ii][jj][kk]
    m1s=mb_mr_data['mp'][f'ensemble{nensemble}'][ii][jj][kk]*alttc/fm2GeV
    ...
    np.save("./temp/dict/"+prefix+"_"+baryon+"_m_eff.npy",meff_data)
    ...
    # if not two_state_enabled:
    if baryon == 'PION' or baryon == 'ETA_S' or baryon == 'ETA_C'  or baryon == 'JPSI' or (baryon == 'PHI' and nensemble!=12):
    # if baryon == 'PION':
