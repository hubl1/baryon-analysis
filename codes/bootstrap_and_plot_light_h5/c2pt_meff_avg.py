...
import warnings
import os
from params.params import alttc_gv, name,ms_arr,fm2GeV
warnings.filterwarnings("ignore",category=DeprecationWarning)
from filelock import FileLock
import re
from utils.c2pt_io_h5 import h5_get_pp2pt
from utils.create_plot import create_plot_ex

def run_meff_avg(
    prefix, ii, nensemble, mp_string, T_strt, T_end,
    tag, baryon, jj, cov_enabled, two_state_enabled, baryon_meson_h5_prefix
):
    ...
    # fm2GeV = 0.1973
    # mPS=gv.gvar(mp_string)

    # c2pt,confs=h5_get_pp2pt(f'../baryon_meson_data_h5/{tag}/c2pt_{prefix}.wp.{baryon}.h5')
    if nensemble == 5 and baryon in ['PROTON', 'DELTA']:
        modified_prefix=re.sub(r"ms-0\.\d+", "ms-0.2310", prefix)
        c2pt, confs=h5_get_pp2pt(f'{baryon_meson_h5_prefix}/baryon_meson_data_h5/{tag}/c2pt_{modified_prefix}.wp.{baryon}.extra.h5')
    else:
    ...
    # c2pt_avg_jcknf = c2pt_bwrd_jcknf
    ...
    def fndroot(eqnf,ini):
        ...
        return sol
    m_eff_cosh_jcknf = np.array([fndroot(eff_mass_eqn(c2pt),0.2*np.ones_like(t_ary)) for c2pt in c2pt_avg_jcknf])*fm2GeV/alttc
    ...
    c1s=mb_mr_data['c1'][f'ensemble{nensemble}'][ii][jj]
    m1s=mb_mr_data['mp'][f'ensemble{nensemble}'][ii][jj]*alttc/fm2GeV
    if not two_state_enabled:
        c2s=np.zeros(4000)
        m2s=np.zeros(4000)
    else:
        c2s=mb_mr_data['c2'][f'ensemble{nensemble}'][ii][jj]
        m2s=mb_mr_data['mp2'][f'ensemble{nensemble}'][ii][jj]*alttc/fm2GeV
    ...
    meff_data['avg_cntrl']= m_eff_cosh_cntrl
    meff_data['avg_err']=m_eff_cosh_err
    ...
    np.save("./temp/dict/"+name[nensemble]+"_"+prefix+"_"+baryon+"_m_eff.npy",meff_data)
    ...
    # if not two_state_enabled:
    if baryon == 'PION'  or baryon == 'KAON' or baryon == 'ETA_S' or (baryon == 'PHI' and nensemble!=12):
    # if baryon == 'PION':
        meff_plot_cntrl=m_eff_cosh_cntrl
        meff_plot_err=m_eff_cosh_err
    ...
    create_plot_ex(0.8/alttc,np.linspace(T_strt, T_end, 1200),np.linspace(0-0.5, T_hlf-0.5, 1200),x=np.array(range(1, T_hlf)),
                ...
                y_lim_strt=0,
                y_lim_end=0,
                # y_lim_strt=mPS.mean-0.2,
                # y_lim_end=mPS.mean+0.2,
                xlabel=r'$t/a$',
                ylabel=r'$m_{\mathrm{eff}}(GeV)$',
                label1=label1,
                label2="best fit",
                prefix=name[nensemble]+"_"+prefix+"_"+baryon+"_m_eff_real",
                T_hlf=T_hlf,
