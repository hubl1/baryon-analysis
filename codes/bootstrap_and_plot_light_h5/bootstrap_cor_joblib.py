...
from c2pt_meff_avg import run_meff_avg
...
baryon_meson_h5_prefix = sys.argv[11]
...
if nensemble == 5 and baryon in ['PROTON', 'DELTA']:
    ...
    pp2pt_loc, confs=h5_get_pp2pt(f'{baryon_meson_h5_prefix}/baryon_meson_data_h5/{tag}/c2pt_{modified_prefix}.wp.{baryon}.extra.h5')
else:
...
T_strt, T_end, ini_p0, two_state_enabled = read_ini_v2(
    nensemble,
    ii_uni,
    # os.path.expanduser("./json_hand/" + baryon + "_found_ini.json"),
...
# save_ini(nensemble,ii,T_strt,T_end,1,fit.Q,"./"+baryon+"_found_ini.json",fit.p,two_state_enabled)
...
if not two_state_enabled:
    mb_mr_temp = {
        ...
        "mp2": -np.ones((n_bootstrap), dtype=float),
        "c2": -np.ones((n_bootstrap), dtype=float),
    }
else:
    mb_mr_temp = {
        "mp": np.zeros((n_bootstrap), dtype=float),
...
with FileLock(lock_file):
    try:
        ...
        default_keys = {
            i: {j: -1 * np.ones(n_bootstrap) for j in range(3, 6)} for i in range(3)
        }
        mb_mr_data = {
...
# print('ii,jj=',ii,jj)
mb_mr_data["mB"][f"ensemble{nensemble}"][ii][jj] = mB
mb_mr_data["T_strt"][f"ensemble{nensemble}"][ii][jj] = T_strt
mb_mr_data["T_end"][f"ensemble{nensemble}"][ii][jj] = T_end
mb_mr_data["mp"][f"ensemble{nensemble}"][ii][jj] = mb_mr_temp["mp"]
mb_mr_data["c1"][f"ensemble{nensemble}"][ii][jj] = mb_mr_temp["c1"]
if two_state_enabled:
    mb_mr_data["mp2"][f"ensemble{nensemble}"][ii][jj] = mb_mr_temp["mp2"]
    mb_mr_data["c2"][f"ensemble{nensemble}"][ii][jj] = mb_mr_temp["c2"]
...
####################
output_txt+=f'\nbootstrap mp = {gv.gvar(np.mean(mb_mr_temp["mp"]),np.std(mb_mr_temp["mp"]))}'
img = Image.new(
