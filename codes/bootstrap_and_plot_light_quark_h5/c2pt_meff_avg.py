...
# ii=int(sys.argv[2])
# jj=int(sys.argv[11])
# T_strt = int(sys.argv[5]) #? starting point of the fit
# T_end = int(sys.argv[6])-1 #? ending point of the fit
...
def run_meff_avg_light_quark(
    prefix, ii, nensemble, mp_string, T_strt, T_end,
    tag, baryon, jj, cov_enabled,  baryon_meson_h5_prefix
):
    ...
    def fndroot(eqnf,ini):
        ...
        return sol
    m_eff_cosh_jcknf = np.array([fndroot(eff_mass_eqn(c2pt),0.12*np.ones_like(t_ary)) for c2pt in c2pt_avg_jcknf])*fm2GeV/alttc
    ...
    c1s=mb_mr_data['Zwp'][f'ensemble{nensemble}'][ii][jj]
    m1s=mb_mr_data['mPS'][f'ensemble{nensemble}'][ii][jj]*alttc/fm2GeV
    # print(m1s)
