...
from c2pt_meff_avg import run_meff_avg_charm_quark
...
pp2pt_loc, confs1=h5_get_pp2pt(f'{baryon_meson_h5_prefix}/baryon_meson_data_h5/{tag}/c2pt_{prefix}.wp.{baryon}.h5')
wwpp2pt_loc, confs2=h5_get_pp2pt(f'{baryon_meson_h5_prefix}/baryon_meson_data_h5/{tag}/c2pt_{prefix}.ww.{baryon}.h5')
a4p2pt_loc, confs3=h5_get_pp2pt(f'{baryon_meson_h5_prefix}/baryon_meson_data_h5/{tag}/c2pt_{prefix}.wp.{baryon}_A4.h5')
confs=list(set(confs1)&set(confs2)&set(confs3))
print(pp2pt_loc.shape)
print(wwpp2pt_loc.shape)
print(a4p2pt_loc.shape)
# exit()
...
# print(pp2pt.shape)
# print(confs.shape)
print('json_confs',set(common_conf_data[f'ensemble{nensemble}']).difference(set(confs)))
print('h5_confs',confs)
# exit()
index_conf=[ i for i in range(len(confs)) if confs[i] in common_conf_data[f'ensemble{nensemble}']]
print(index_conf)
print(len(index_conf))
# exit()
n_bootstrap = 4000
pp2pt=pp2pt_loc[index_conf,:]
wwpp2pt=wwpp2pt_loc[index_conf,:]
a4p2pt=a4p2pt_loc[index_conf,:]
...
# exit()
# a4p2pt = np.load("./Data/"+prefix+".a4p2pt.npy")/ns_arr[nensemble]**3
# wwpp2pt = np.load("./Data/"+prefix+".ww.pp2pt.npy")/ns_arr[nensemble]**3
# wwa4p2pt = np.load("./Data/"+prefix+".ww.a4p2pt.npy")
# pp2pt=(pp2pt[::3,:]+pp2pt[1::3,:]+pp2pt[2::3,:])/3
# a4p2pt=(a4p2pt[::3,:]+a4p2pt[1::3,:]+a4p2pt[2::3,:])/3
# wwpp2pt=(wwpp2pt[::3,:]+wwpp2pt[1::3,:]+wwpp2pt[2::3,:])/3
# block_average and reduce the bootstrap resources needed
...
def check_and_create_file(nensemble):
    # dir_path = "../../bootstrap_and_plot_charm/indices"
...
# print(combined_data_cov.shape)
# print(fake_combined_cov.shape)
...
print(output_txt)
# print(fit.p['mPS'])
# print(gv.mean(fit.p["mPS"]) * fm2GeV / alttc)
# print(gv.sdev(fit.p["mPS"]) * fm2GeV / alttc)
# print(gv.sdev(fit.p["mPS"]) * fm2GeV / alttc/2)
...
if not os.path.exists("./temp"):
    os.makedirs("./temp")
img.save(
    "./temp/"+name[nensemble]+"_"+prefix+"_"+baryon+ "_fit_params.png"
)
...
mb_mr_data["mR"][f"ensemble{nensemble}"][ii][jj][kk] = mb_mr_temp["mR"]
mb_mr_data["mPS"][f"ensemble{nensemble}"][ii][jj][kk] = mb_mr_temp["mPS"]
mb_mr_data["fpi"][f"ensemble{nensemble}"][ii][jj][kk] = mb_mr_temp["fpi"]
mb_mr_data["Zwp"][f"ensemble{nensemble}"][ii][jj][kk] = mb_mr_temp["Zwp"]
...
# os.system('./c2pt_meff_avg.py '+(str(tag)+' '+str(ii)+' '+str(nensemble)+' "'+str(gv.gvar(res_cntrl_err['mPS_cntrl'],res_cntrl_err['mPS_err']))+'"'+' '+str(T_strt)+' '+str(T_end))+' '+str(mB)+' '+str(1)+' '+prefix+' '+baryon+' '+str(jj)+' '+str(int(cov_enabled))+' '+str(kk))
mp_gvar = gv.gvar(res_cntrl_err["mPS_cntrl"], res_cntrl_err["mPS_err"])
run_meff_avg_charm_quark(
    prefix,
    ii,
    nensemble,
    str(mp_gvar),
